(function ($) {
    ...
    function dispatchTimer() {
        var a = (settings.end - settings.now) / (1000 * 60 * 60 * 24);
        var b = 24 - 24 * (a - Math.floor(a));
        var c = 60 - 60 * (Math.ceil(b) - b); 
        var d = 60 - 60 * (Math.ceil(c)- c);
        timer = {
            total: Math.floor((settings.end - settings.start) / 86400),
            days: Math.floor(a),
            hours: Math.ceil(b),
            minutes: Math.floor(c),
...
//            seconds: 60 - Math.floor((((settings.end - settings.now) % 86400) % 3600) % 60)
    ...
    function prepareCounters() {
        ...
        var minutesStage = new Kinetic.Stage({container: settings.selectors.canvas_minutes, width: minutes_width, height: minutes_width});

        circleMinutes = new Kinetic.Shape({drawFunc: function (context) {
                var minutes_width = $('#' + settings.selectors.canvas_minutes).width();
            ...
            }, stroke: settings.minutes.borderColor, strokeWidth: settings.minutes.borderWidth});
        ...
        var hoursStage = new Kinetic.Stage({container: settings.selectors.canvas_hours, width: hours_width, height: hours_width});
        circleHours = new Kinetic.Shape({drawFunc: function (context) {
                var hours_width = $('#' + settings.selectors.canvas_hours).width();
            ...
            }, stroke: settings.hours.borderColor, strokeWidth: settings.hours.borderWidth});
        ...
        var daysStage = new Kinetic.Stage({container: settings.selectors.canvas_days, width: days_width, height: days_width});
        circleDays = new Kinetic.Shape({drawFunc: function (context) {
                var days_width = $('#' + settings.selectors.canvas_days).width();
            ...
            }, stroke: settings.days.borderColor, strokeWidth: settings.days.borderWidth});
